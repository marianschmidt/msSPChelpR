...
reshape_long_dt <- function(wide_df, case_id_var, time_id_var, chunks = 2, datsize = Inf){
  ...
  long_df <- long_df %>%
    ...
    lapply(., stats::reshape, timevar=time_id_var, idvar=case_id_var, direction = "long", sep=".") %>%

...
reshape_wide_tt <- function(df, case_id_var, time_id_var, timevar_max = 6, datsize = Inf){
  ...
  if(max_time > timevar_max){
    ...
    df <- df %>%
      ...
      tidytable::arrange.(!!rlang::sym(case_id_var),!!rlang::sym(time_id_var))%>%
      #calculate new renumbered variable group by case_id_var
      tidytable::mutate.(counter = dplyr::row_number(), by = !!rlang::sym(case_id_var)) %>%
  ...
  df %>% tidytable::pivot_wider.(
    ...
    #sort by case_id_var
    tidytable::arrange.(!!rlang::sym(case_id_var))

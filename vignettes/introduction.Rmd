...
usdata_wide <- usdata_wide %>%
  dplyr::mutate(p_spc = case_when(is.na(t_site_icd.2)   ~ "No SPC",
                         ...
                         TRUE ~ NA_character_)) %>%
  #create the same information as numeric variable
  dplyr::mutate(spc_count = case_when(is.na(t_site_icd.2)   ~ 1,
                            TRUE ~ 0))
usdata_wide %>%
   select(fake_id, sex.1, p_spc, spc_count, t_site_icd.1, t_datediag.1, t_site_icd.2, t_datediag.2)

...
It is recommended to run the following steps in the correct order to obtain accurate follow-up time calculations [Step.
 ...
 4. [Set flag for Second Primary Cancer diagnosis]: 
 
 4. [Determine patient status](#step-pat-status) at a defined end of follow-up by using the `msSPChelpR::pat_status()` function. This date for end of follow-up must:
 ...
 5. [Calculate follow-up time](#step-calc-futime) for the same dataset by using the `msSPChelpR::calc_futime()` function and the same `fu_end` as for step 4. By standard all functions of the `msSPChelpR` package require follow-up times as numeric years.
...
### Step 5 - Determine patient status at end of FU {#step-pat-status}

### Step 6 - Calculate FU time {#step-calc-futime}

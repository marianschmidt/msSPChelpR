...
renumbered_usdata %>%
   select(fake_id, sex, site_icd, t_datediag, t_tumid)
```
...
usdata_wide <- renumbered_usdata %>%
  msSPChelpR::reshape_wide_tt(case_id_var = "fake_id", time_id_var = "t_tumid", timevar_max = 10)
...
usdata_wide <- usdata_wide %>%
  dplyr::mutate(p_spc = case_when(is.na(site_icd.2)   ~ "no SPC",
                         !is.na(site_icd.2)           ~ "SPC developed",
                         TRUE ~ NA_character_)) 
usdata_wide %>%
   select(fake_id, sex.1, p_spc, site_icd.1, t_datediag.1, site_icd.2, t_datediag.2)

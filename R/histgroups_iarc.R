...
histgroup_iarc <- function(df, hist_var, new_var_hist = t_histgroupiarc, version = "3.2"){
  
  
  #check for valid revision used
  if (!version %in% c("3.0", "3.1", "3.2") | !is.character(version)) {
    rlang::abort(
      c("You have specified an invalid `version` parameter",
        "x" = paste0("Version", version, " is not valid."),
        "i" = "`version` must be either \"3.1\" for ICD-O-3 revision 1, or \"3.2\" for ICD-O-3 revision 2."
      )
    )
  }

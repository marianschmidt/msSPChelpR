...
sir <- function(df,
  ...
  for(y in 1:length_yb){
    ...
    #F1b observed
    sircalc_count <- df %>%
      {if (!xb & !yb){dplyr::group_by(., .data$age, .data$sex, .data$region, .data$year, .data$t_icdcat)} else{.}} %>%
      {if (!xb & yb){dplyr::group_by(., .data$age, .data$sex, .data$region, .data$year, .data$t_icdcat, !!syb_var)} else{.}} %>%
    ...
    sircalc_count <- sircalc_count %>% #complete groups where i_observed = 0
      ...
      {if (!xb & yb){tidyr::complete(., .data$age, .data$sex, .data$region, .data$year, .data$t_icdcat, !!syb_var)} else {.}} %>%
    ...
    sircalc_fu <- df %>%
      {if (!xb & !yb){dplyr::group_by(., .data$age, .data$sex, .data$region, .data$year)} else{.}} %>%
      {if (!xb & yb){dplyr::group_by(., .data$age, .data$sex, .data$region, .data$year, !!syb_var)} else{.}} %>%
    ...
    sircalc <- sircalc_count %>%
      dplyr::full_join(sircalc_fu, by = c("age", "sex", "region", "year", if(yb){rlang::expr_text(syb_var)}))

...
usdata_wide <- usdata_wide %>%
  dplyr::mutate(p_spc = case_when(is.na(t_site_icd.2)   ~ "no SPC",
                         ...
                         TRUE ~ NA_character_)) 
usdata_wide %>%
   select(fake_id, sex.1, p_spc, t_site_icd.1, t_datediag.1, t_site_icd.2, t_datediag.2)
...
usdata_wide %>%
   select(fake_id, p_status, p_alive.1, datedeath.1, t_site_icd.1, t_datediag.1, t_site_icd.2, t_datediag.2)
...
usdata_wide %>%
   select(fake_id, p_status, p_futimeyrs, p_alive.1, datedeath.1, t_datediag.1, t_datediag.2)
